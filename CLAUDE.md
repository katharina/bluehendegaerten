...
- **Every badge that represents "you are here" scrolls back to the top of its own level when clicked.** This is a repeating pattern — always wire it in for any new badge added to a top-nav breadcrumb row, without waiting to be asked. Examples: BG square → `window.scrollTo({ top: 0 })`; `.garden-name-sticky` on the garden page → scrolls its own scroll container to top; a level's badge on a page where that level owns the in-flow title (e.g. `#obs-title-sticky` on the Beobachtungen page) → `window.scrollTo({ top: 0 })`. If the badge instead navigates to a different page (e.g. `.garden-name-sticky` on the Beobachtungen breakout page linking back to the garden page), the scroll-to-top rule doesn't apply — only add it when the badge's own title lives in-flow on the current page. Remember to also set `pointer-events: auto` and `cursor: pointer` on the visible state, since badge base styles default to `pointer-events: none`.
